...
def upgrade():
    ...
    engine.execute(text(
        "ALTER TABLE newsletters ADD COLUMN updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP"
    ))
    
    # Add index to newsletters.created_at
    engine.execute(text(
        "CREATE INDEX IF NOT EXISTS ix_newsletters_created_at ON newsletters (created_at)"
    ))
