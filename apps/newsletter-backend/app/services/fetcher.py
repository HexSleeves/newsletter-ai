...
class ArticleFetcher:
    ...
    def fetch_all(self) -> list[Article]:
        ...
        for feed_url in self.settings.rss_feed_list:
            try:
                feed_articles = self.fetch_from_feed(feed_url)
    ...
    def fetch_from_feed(self, feed_url: str) -> list[Article]:
        ...
        try:
            ...
            for entry in feed.entries:
                try:
                    article = self._process_entry(entry, feed_url, feed)
                    if article:
    ...
    def _process_entry(self, entry, feed_url: str, feed) -> Article | None:
